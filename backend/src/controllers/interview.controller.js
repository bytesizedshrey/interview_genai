  ...
  try {
    ...
    if (req.file) {
      try {
        ...
        console.error("Standard pdf-parse failed, trying alternative:", err);
        try {
          const parsed = new pdfParse.PDFParse(Uint8Array.from(req.file.buffer));
          const resVal = await parsed.getText();
          resumeText = resVal.text || resVal || '';
        } catch (err2) {
          console.error("Alternative pdf-parse failed:", err2);
        }
      }
    }
